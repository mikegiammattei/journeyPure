...
function paginationLikedPosts() {
    const obj = {
        postPaged : $('[data-pagination-paged]'),
    };
    let paged = 1;
    let get = 0;
    let offset = $('[data-pagination-paged]').data('pagination-show-per-page');
    obj.postPaged.on('click',function () {

        let inMotion  = true;
        let $thisOffsetBtn = $(this);
        ...
        if(get === 1){
            $.ajax({
                ...
                success:function(data) {
                    ...
                    $('.top-posts .fill-posts-pagination').append(data);
                    paged++;
