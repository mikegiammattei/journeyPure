jQuery(document).ready(function () {
	if(jQuery('[data-target="#user-question-form-container"]').length > 0 ||  jQuery('#chat-ctm-btn').length > 0){
		...
		if(jQuery(window).outerWidth() > 767){
			jQuery('[data-target="#user-question-form-container"]').add('#chat-ctm-btn').css({'cursor ':'pointer'}).attr('disabled','disabled').attr('data-placement','top').attr('data-html','true').attr('data-trigger', 'click').attr('title', 'Text <span>(855) 952-0303</span> on your mobile phone to chat with us now.').tooltip('update');
...
function userQuestionClientForm(){
	...
	let formEl = jQuery('[data-user-form="ask question"]');

		jQuery(formEl).find('.alert').css({"display": "none"});
