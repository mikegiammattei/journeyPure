 ...
 *    6. Minifies the CSS file and generates style.min.cssd
...
gulp.task( 'scriptDefer', () => {
	return gulp
		.src( config.jsDeferSRC, { since: gulp.lastRun( 'scriptDefer' ) }) // Only run on changed files.
		.pipe( plumber( errorHandler ) )
		.pipe(
			babel({
				presets: [
		...
		.pipe( remember( config.jsDeferSRC ) ) // Bring all files back to stream.
		.pipe( concat( config.jsDeferFile + '.js' ) )
		.pipe( lineec() ) // Consistent Line Endings for non UNIX systems.
		.pipe( gulp.dest( config.jsDeferDestination ) )
		.pipe(
			rename({
				basename: config.jsDeferFile,
				suffix: '.min'
			})
		)
		.pipe( uglify() )
...
gulp.task('minify', () => {
	return gulp.src(config.minifySRC)
		.pipe( htmlmin({
			...
			ignoreCustomFragments: [/<%[\s\S]*?%>/, /<\?[\s\S]*?\?>/,/\<\/body\>/,/\<\/html\>/,/\<div class\=\"main\-wrapper\"\>/,/\<\/div\>/]
...
gulp.task(
	'default',
	gulp.parallel( 'styles', 'vendorsJS', 'customJS',/*'scriptDefer',*/ 'minify', 'images', browsersync, () => {
		gulp.watch( config.watchPhp, reload ); // Reload on PHP file changes.
		gulp.watch( config.watchPhp, gulp.parallel( 'minify' ) ); // Reload on minify file changes.
		gulp.watch( config.watchStyles, gulp.parallel( 'styles' ) ); // Reload on SCSS file changes.
		//gulp.watch( config.watchJsVendor, gulp.parallel( 'vendorsJS' ) ); // Reload on SCSS file changes.
		gulp.watch( config.watchJsCustom, gulp.parallel( 'customJS' ) ); // Reload on SCSS file changes.
		//gulp.watch( config.watchJsDefer, gulp.parallel( 'scriptDefer' ) ); // Reload on SCSS file changes.
		//gulp.watch( config.watchJsVendor, gulp.series( 'vendorsJS', reload ) ); // Reload on vendorsJS file changes.
		gulp.watch( config.watchJsCustom, gulp.series( 'customJS', reload ) ); // Reload on customJS file changes.
		//gulp.watch( config.watchJsDefer, gulp.series( 'scriptDefer', reload ) ); // Reload on DeferJS file changes.
